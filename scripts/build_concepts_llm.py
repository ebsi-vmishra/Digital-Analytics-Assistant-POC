...
def _docs_index_by_table(docs_rows: List[Dict]) -> Dict[str, Dict]:
    # Each row: {"table":"schema.table", "description":"...", "columns":[ ... ]}
    out = {}
...
# ------------------------ heuristic backfill (keeps you non-empty) ------------------------
...
def _heuristic_concepts(schema_subset: Dict, docs_subset: List[Dict], tenant_id: str) -> Dict[str, List[Dict]]:
    """
    If the LLM returns nothing for a batch, seed minimal concepts by scanning names/descriptions
    for a few high-signal business notions (smoker/tobacco, employment type).
    """
    ...
    for d in docs_subset:
        ...
        for c in cols:
            ...
            if _SMOKER_PAT.search(c_text):
                add_concept("C_SMOKER", "Tobacco/Smoker Indicator", "Indicates whether a member uses tobacco.", "member")
                add_attr("C_SMOKER", t, colname, role="attr",
            ...
            if _EMPLOY_PAT.search(c_text):
                add_concept("C_EMPTYPE", "Employment Type", "Full-Time/Part-Time style classification.", "member")
                add_attr("C_EMPTYPE", t, colname, role="attr",
...
def _build_prompt(
    ...
    """
    Provide tightly-scoped payloads to the model with explicit return spec.
    """
    return f"""
You are a meticulous data analyst creating a Concept Layer for tenant "{tenant_id}".
Cluster columns and their documented meanings into stable business concepts across the tables in this batch.
Normalize common variants (e.g., Smoker/TobaccoUser; EmploymentType FT/PT vs Full-Time/Part-Time).
...
TASK
1) concept_catalog: id, name, description, grain (employee/dependent/enrollment/etc), owner (optional), status="draft".
...
3) concept_attributes: map actual columns to concepts with: tenant_id "{tenant_id}", table, column, role ("key"|"attr"|"metric"), optional transform_sql for normalization (e.g., FT/PT → 'Full-Time'/'Part-Time'; Y/N or 0/1 → TRUE/FALSE).
...
5) Do not invent tables/columns.
...
def _normalize_rows(data: Dict[str, Any], tenant_id: str) -> Dict[str, List[Dict]]:
    """
    Make LLM output robust:
      - Map common key variants to our schema (e.g., col/column_name -> column, tbl/table_name -> table).
      - Coerce field types and fill defaults.
      - Generate concept_id from concept_name when missing.
    """
    out = {
        "concept_catalog": [],
        "concept_alias": [],
    ...
    # ---- Attributes (accept 'col', 'column_name', 'tbl', 'table_name', etc.)
    for r in (data.get("concept_attributes") or []):
        col = (r.get("column") or r.get("col") or r.get("column_name") or "").strip()
        tbl = (r.get("table")  or r.get("tbl") or r.get("table_name")  or "").strip()
...
def build_concepts(cfg: Dict) -> Tuple[str, str, str, str]:
    ...
    # Batching (+ optional limit)
    bsz = int(cfg["llm"].get("concepts_batch_size", 10))
    # apply an optional table limit if present (keeps compatibility with older configs)
    ...
    for bi, tables in enumerate(table_batches, start=1):
        ...
        prompt = _build_prompt(tenant_id, schema_subset, relationships, docs_subset)
        ...
        # If empty, seed heuristic backfill
        ...
        # ---- Normalize LLM output so we never KeyError on 'col' etc.
        data_norm = _normalize_rows(data, tenant_id)
    ...
    # Fail-fast if truly nothing (shouldn’t happen due to backfill + normalization)
